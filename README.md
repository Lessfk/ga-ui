...
ga-ui-plus 是一个基于 Vue 3 和 Element Plus 的组件库工作区。本仓库包含组件库源码、构建与契约验证脚本，以及用于本地联调的 Playground。

## Workspace 概览

- `packages/ui-element`：组件库 npm 包目录，包名为 `ga-ui-plus`。
- `playground`：引用 workspace 内 `ga-ui-plus` 的 Vue 3 示例应用。
- `docs`：项目文档目录。

组件库提供三个公开 JavaScript/TypeScript 入口：

- `ga-ui-plus/base`：基础组件 `GaTable`、`GaPagination` 及相关类型。
- `ga-ui-plus/business`：业务组件 `GaTablePagination` 及相关类型。
- `ga-ui-plus`：聚合基础组件和业务组件的根入口。

样式统一从 `ga-ui-plus/style.css` 导入；Element Plus 的样式需要由消费者单独导入。
...
```bash
npm install ga-ui-plus vue element-plus
```

消费者项目需要提供 `ga-ui-plus` 的 peer dependencies：`vue` 与 `element-plus`。
...
`packages/ui-element` 下的 `ga-ui-plus` npm 包使用 [MIT License](packages/ui-element/LICENSE)。此说明仅适用于该 npm 包，不代表整个 workspace 采用 MIT License。
